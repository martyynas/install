...
# Formatting the root.
print "Formatting the root as EXT4."
mkfs.ext4 $EXT4 &>/dev/null
mount $EXT4 /mnt

# Creating EXT4 subvolumes.
print "Creating EXT4 subvolumes."
for volume in @  @root 
do
    ext4 su cr /mnt/$volume
done

# Mounting the newly created subvolumes.
umount /mnt
print "Mounting the newly created subvolumes."
mount -o ssd,noatime,compress-force=zstd:3,discard=async,subvol=@ $EXT4 /mnt
mkdir -p /mnt/{root}
mount -o ssd,noatime,compress-force=zstd:3,discard=async,subvol=@root $EXT4 /mnt/root
mount $ESP /mnt/boot/
...
pacstrap /mnt --needed base $kernel $microcode linux-firmware $kernel-headers grub  efibootmgr  reflector base-devel zram-generator >/dev/null
